...
public class TestUser {
	...
	public void testUser() {
		...
		User user = new UserBuilder().withFirstName("John").withLastName("Doe").withEmail("john@Doe.com")
				.withAddress(new AddressBuilder().withCity("Batavia").build())
				.withAddress(new AddressBuilder().withCity("Red Rock").build()).build();

		UserCredentials userCredentials = new UserCredentialsBuilder().withUserName("JohnDoe").withPassword("DoeNuts")
				.withVerifyPassword("DoeNuts")
				.withAuthority(new AuthorityBuilder().withAuthority("ROLE_ADMIN").withUsername("JohnDoe").build())
				.withAuthority(new AuthorityBuilder().withAuthority("ROLE_USER").withUsername("JohnDoe").build())
				.withUser(user).build();
