...
import edu.mum.cs.cs544.exercises.models.Airline;
import edu.mum.cs.cs544.exercises.models.Airplane;
import edu.mum.cs.cs544.exercises.models.Airport;
...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            @SuppressWarnings("unchecked")
            List<Airline> airlines = session.createQuery("select p from Airline p join p.flights b where b.airplane.model = 'A380'").list();
        ...
        try {
            ...
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("from Flight f where f.airplane.model = '747' " +
        ...
        try {
            ...
            List<Flight> flights = session.createQuery("from Flight f where " +
                    "f.departureDate = '2009-08-07' and f.departureTime < '12:00:00'").list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
