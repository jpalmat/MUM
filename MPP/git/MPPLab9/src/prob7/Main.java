...
public class Main {
	...
	public static void ordering1(List<Integer> list) {
		Comparator<Integer> byAbsoluteValue = (x, y) -> Math.abs(x) - Math.abs(y); 
		Comparator<Integer> byIncreasingOrder = (x, y) -> x - y; 
		System.out.println(list.stream().sorted(byAbsoluteValue.thenComparing(byIncreasingOrder)).collect(Collectors.toList()));
	...
	public static void ordering2(List<String> words) {
		Comparator<String> reverseAndCompare = (x, y) -> new StringBuilder(x).reverse().toString()
													.compareTo(new StringBuilder(y).reverse().toString()); 
		System.out.println(words.stream().sorted(reverseAndCompare).collect(Collectors.toList()));
