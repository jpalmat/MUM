...
//		m.displayAllOrders();
	...
	private void showAllOrderItems() {
		System.out.println("\n==============\nAll order items:");
		List<OrderItem> p = orders.stream().map(x -> x.getOrderItems()).flatMap(Collection::stream).collect(Collectors.toList());
		System.out.println(p);
		
		System.out.println("\n==============\nAll order itemsssssss:");
		//implement
		
		Comparator<OrderItem> con = Comparator.comparing(OrderItem::getUnitPrice, (o1, o2)-> o1==o2? 0 : o1 < o2 ? -1 : 1);
		List<OrderItem> a = orders.stream().flatMap(o -> o.getOrderItems().stream()).sorted(con).collect(Collectors.toList());
