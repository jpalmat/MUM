...
public class Employee {
	...
	public static void main(String[] args) {
		...
		Comparator<Employee> con = Comparator.comparing(Employee::getName, (e1, e2) -> e1.compareTo(e2));
		Comparator<Employee> con1 = Comparator.comparing(Employee::getSalary, (e1, e2) -> e1==e2 ? 0 : e2 < e1 ? -1 : 1);
		//expected output:
		//[[Jim, 100000], [Jim, 75000], [Jim, 70000], [Joe, 59000], [Joe, 50000], [Rich, 88000], [Steve, 55000], [Tom, 80000]]
		System.out.println(list.stream()
				.sorted(Comparator.comparing(bySalary).reversed())
				.sorted(Comparator.comparing(byName))
				.collect(Collectors.toList()));
		
		System.out.println(list.stream().sorted(con.thenComparing(con1)).collect(Collectors.toList()));
	}
}
